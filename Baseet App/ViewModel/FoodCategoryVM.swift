...
class FoodCategoryVM{
    ...
    var categoryRestaurantNameArray = [String]()
    var deliveryTimeArray = [String]()
    var ratingArray = [Int]()
    var imageArary = [String]()
    var applogoArray = [String]()
    ...
    func getRestaurants(){
        if Reachability.isConnectedToNetwork(){
                ...
                print(headers)
            print("\(Constants.Common.finalURL)/categories/restaurants/\(categoryID!)")
                AF.request("\(Constants.Common.finalURL)/categories/restaurants/\(categoryID!)", method: .get, encoding: JSONEncoding.default,headers: headers)
                        .responseJSON { response in
                            ...
                            case .success(let json):
                                ...
                                self.categoryRestaurantNameArray.removeAll()
                                self.deliveryTimeArray.removeAll()
                                self.ratingArray.removeAll()
                                self.imageArary.removeAll()
                                self.applogoArray.removeAll()
                                let banners = response["restaurants"] as? Array<Any>
                                for restList in banners!{
                                    let dict = restList as! [String:AnyObject]
                                    let id = dict["id"] as? Int ?? 0
                                    let applogo = dict["applogo"] as? String ?? ""
                                    let name = dict["name"] as? String ?? ""
                                    let appcoverlogo = dict["appcoverlogo"] as? String ?? ""
                                    let rating_count = dict["rating_count"] as? Int ?? 0
                                    let delivery_time = dict["delivery_time"] as? String ?? ""
                                    self.categoryIDArray.append(id)
                                    self.categoryRestaurantNameArray.append(name)
                                    self.imageArary.append(appcoverlogo)
                                    self.applogoArray.append(applogo)
                                    self.ratingArray.append(rating_count)
                                    self.deliveryTimeArray.append(delivery_time)
                                    DispatchQueue.main.async {
                                        self.reloadtabRestaurantloader?()
                                    }
                                }
    ...
    func makeBannerCall(){
        if Reachability.isConnectedToNetwork(){
            self.showLoadingIndicatorClosure?()
                ...
                print(headers)
                AF.request("\(Constants.Common.finalURL)categories/restaurants/3", method: .get, encoding: JSONEncoding.default,headers: headers)
                        .responseJSON { response in
                            switch response.result {
                            case .success(let json):
                                print(json)
